...
Here the error message is telling us that we need an `initial()` call for `R` as we have an `update()` call for it.  All error messages include a prompt to run `odin2::odin_error_explain("Exxxx")` for more information where `Exxxx` will be the error code for the specific type of error (the full list of error codes can be found [here](https://mrc-ide.github.io/odin2/articles/errors.html)).  Running in this case gives
