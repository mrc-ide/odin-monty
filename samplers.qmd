...
In `monty`, parallel tempering is implemented in the function `monty_sampler_parallel_tempering()` based on the non-reversible swap approach [@syed_non-reversible_2022]. We will use the RW sampler to sample within each chain, but other samplers can also be used.
