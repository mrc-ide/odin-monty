...
We can see that of our three chains, one is only exploring the mode around $x = 5$, while the other two are only exploring the mode around $x = -5$. So $\frac{1}{3}$ of our samples are from one mode and $\frac{2}{3}$ the other, which we know is not representative of the true distribution! So even if running multiple chains manages to help identify and explore more than one local mode, they cannot tell you anything about the relative densities of those modes if each chain ends up exploring a single mode. We need a better way to make use of multiple chains, and that's where parallel tempering comes in.
