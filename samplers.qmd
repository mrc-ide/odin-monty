...
2. **Adaptive Scaling**  
   - The sampler also updates a **global scaling factor** (multiplied by $2.38^2 / d$ in the proposal, where $d$ is the number of parameters) to target a desired acceptance rate (`acceptance_target`).  
...
## Details of the adaptive algorithm
...
est_vcv <- res_adaptive$details[[1]]$vcv
print(est_vcv)
```
...
```{r}
plot(res_adaptive$details[[1]]$scaling_history, type = "l", xlab = "Iteration",
     ylab = "Scaling factor")
```
