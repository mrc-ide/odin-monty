...
In `monty`, parallel tempering is implemented in the function `monty_sampler_parallel_tempering()` based on the non-reversible swap approach [@syed_non-reversible_2022]. Currently, this sampler uses a random-walk Metropolis step (`monty_sampler_random_walk`) within each chain for local exploration, but we plan to allow other samplers in future releases.
