...
The `monty_sampler_random_walk()` function allows us to define an RWMH sampler by passing the Variance-Covariance matrix of the distribution as an argument.
...
We can see from the autocorrelation function that we have to wait almost 100 steps for the algorithm to produce a “new” (i.e. not correlated) sample, meaning a lot of calculation is “wasted.” Improving the number of uncorrelated samples is at the heart of many strategies to improve the efficiency of Monte Carlo sampling algorithms. In particular, the adaptive Metropolis-Hastings sampler offers tools to automatically optimise the proposal distribution. 
...
Tuning HMC involves setting parameters such as the step size (often denoted $\epsilon$) and the number of leapfrog (or integration) steps. A small step size improves accuracy of the leapfrog integrator but increases computational cost, whereas too large a step size risks poor exploration and lower acceptance rates. Similarly, the number of leapfrog steps determines how far the chain moves in parameter space before proposing a new sample. Balancing these factors can initially be more involved than tuning simpler methods like random-walk Metropolis. However, modern approaches - such as the No-U-Turn Sampler (NUTS) - dynamically adjust these tuning parameters, making HMC more user-friendly and reducing the need for extensive manual calibration.

By leveraging gradient information, HMC is often able to navigate complex, high-dimensional, or strongly correlated posteriors far more efficiently than random-walk-based approaches. It typically exhibits much lower autocorrelation in the resulting chains, meaning fewer samples are needed to achieve a given level of accuracy in posterior estimates. As a result, HMC has become a popular default in many modern Bayesian libraries, including Stan, PyMC, and others. Nevertheless, its reliance on smooth gradient information limits its application to models where the target density is differentiable - discontinuous distributions can present challenges. Additionally, while HMC can significantly reduce random-walk behaviour, the additional computations required to evaluate gradients and integrate Hamiltonian dynamics means that it is not always faster in absolute wall-clock terms, especially for models with expensive gradient evaluations.

Overall, HMC provides a powerful framework for exploring complex posterior surfaces. When combined with robust tuning strategies or adaptive methods like NUTS [@hoffman_no-u-turn_2014], it is a leading choice for many real-world Bayesian inference problems, frequently outperforming more traditional MCMC algorithms in efficiency and scalability.
...
ex_mixture <- function(){
  fn <- function(x) {
    log(dnorm(x, mean = 5)/2 + dnorm(x, mean = -5)/2)
  }
  monty_model_function(fn,
                       allow_multiple_parameters = TRUE)
}
likelihood <- ex_mixture()
```

the prior a normal distribution with a wider variance
...
And the posterior simply the product of the two (or the sum if working with log-densities)
```{r}
...
plot(x, y / sum(y) / diff(x)[[1]], col = "red", type = 'l')
```
...
As we can see, the RW sampler does not manage to move outside of the mode where it is initially locked. MCMC theory tells us that it will eventually reach the other side of the distribution, whether with a low probability large move or by accepting many small disadvantageous steps in the right direction. It practices it can mean that the second mode is never explored in finite time.
...
In `monty`, parallel tempering is implemented in the function `monty_sampler_parallel_tempering()`. Currently, this sampler uses a random-walk Metropolis step (`monty_sampler_random_walk`) within each chain, but we plan to allow other samplers in future releases.

```{r}
s <- monty_sampler_parallel_tempering(n_rungs = 10, vcv = matrix(.1))
```
...
A parallel tempering sampler performs more total computations than a single-chain sampler because it runs multiple chains simultaneously (e.g. `n_rungs + 1` chains) with only the "coldest" chain targeting the distribution of interest. However, there are scenarios where this additional cost pays off:
