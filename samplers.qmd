...
We have `boundaries = "reflect"` as the default as we expect this is what users will want in most cases, though there may be situations in which `boundaries = "reject"` is more useful (and quicker, by its nature of skipping a number of density calculations). In situations where you are unlikely to propose out-of-bounds, there will be little difference between the options. 
