...
```{r}
# Read the data
original_data <- rowSums(read.csv(file = "data/andre_estimates_21_02.txt",
                                  sep = "\t"))
...
```{r}
plot(t, dust_unpack_state(mod, y)$incidence, type = "l", col = "#000088ff", lwd = 2)
```
...
lines(t, dust_unpack_state(mod, y)$incidence * pars$rho, col = "#000088ff", lwd = 2)
```
...
This is the likelihood associated with the `dust` model written using the `odin` DSL. We know need to move this into the `monty` statistical world to be able to integrate this into our pipeline. For this, we use the `packer` concept (see @sec-MCMC) that allows us to define a one-to-one translation between the two worlds.
