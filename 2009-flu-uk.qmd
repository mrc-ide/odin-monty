...
```{r}
packer <- monty_packer(c("alpha", "R_0", "rho"),
...
```{r}
vcv <- diag(c(5e-10,5e-5,1e-5))*0.0001
#vcv <- matrix(c(4.3e-12,-1.2e-8,3.6e-9,-1.2e-8,4.9e-5,-1e-5,3.6e-9,-1e-5,1.1e-5), nrow = 3)*0.1
...
init_values <- packer$pack(list(alpha=par0[1],
                  ...
                  h_times = hol_t,
                  h_values= hol_v))
init_values <- c(3e-5,1.35,0.04)
samples <- monty_sample(posterior, sampler, 100000, initial = init_values,
                        n_chains = 3)
```
