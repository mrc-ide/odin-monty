...
```{r}
# Read the data
original_data <- rowSums(read.csv(file = "data/andre_estimates_21_02.txt", sep = "\t"))
...
This is the likelihood associated with the `dust` model written using the `odin` DSL. We know need to move this into the `monty` statistical world to be able to integrate this into our pipeline. For this use the `packer` concept that allows us to define a one-to-one translation between the two worlds.
