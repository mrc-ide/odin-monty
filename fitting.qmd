...
* After running is more flexible

## Thinning {.smaller}

* The benefit of thinning is largely in reducing the size of trajectories
* But you might not want to retain the full chains of the parameters and their densities for diagnostics and traceplots
* With the `save_full_chains` option you can still save these
* These can be found in `full_chains`
...
## Flattening chains {.smaller}

::: {style="font-size: 1.5em"}

The chains dimension of objects can be flattened into the samples dimension while running
...
This can be reversed with `monty_unflatten_chains()`.
...
#| echo: false
y_fit <- dust_unpack_state(filter, samples$observations$trajectories)
incidence <- array(y_fit$incidence, c(150, 200))
