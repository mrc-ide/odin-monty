...
sir_age <- odin({
  ...
  update(R[]) <- R[i] + n_IR[i]
  update(incidence[]) <- incidence[i] + sum(n_SI[])
...
Normally, if we had `cases` as a scalar datastream an example `data.frame` could be setup by writing

```{r}
d <- data.frame(time = c(1, 2, 3, 4),
                cases = c(0, 4, 6, 10))
d
```

while to make a list-column for `cases` as a vector datastream we pass in a *list* with the same number of elements as there are rows, and with the same number of elements *per list element* as there are age groups.  We then have to wrap this with `I()`, for example with 2 age groups
