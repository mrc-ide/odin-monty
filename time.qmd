...
* Often the same system can be expressed as either a set of ODEs or as a set of recurrence equations but will behave differently (see below)
...
::: {.callout-note}
If you have used odin version 1, then discrete time models had a diferent (though fundamentally equivalent) formulation of time.  See the [odin2 migration vignette](https://mrc-ide.github.io/odin2/articles/migrating.html) for more details.
:::
...
```{r}
sys <- dust_system_create(logistic_ode(), list(), n_particles = 1)
...
Now, we write out the same system in discrete time:
...
```{r}
sys2 <- dust_system_create(logistic_map(), list(), n_particles = 1)
